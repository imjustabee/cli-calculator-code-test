...
parser.evaluate = function (expr, env) {
	var c = new parser(expr, env);
	return parser._eval(c.parse(), env);
}

parser._eval = function (node, env) {
	if (node.type === 'number') {
		return node.value;
...
parser.prototype._parseParenExpr = function (tokenArr) {
	return { type: 'expr', subtree: this._parseHelper(tokenArr) }
}

parser.prototype._handleFuncCallsAndVarRefs = function (tokenArr) {
	let idIdx;
	do {
...
parser.prototype._parseFunctionArgs = function (tokenArr) {
	let parenDepth = 0,
		argTokenArrays = [],
		temp = [];
...
parser.prototype._handleParenExpressions = function (tokenArr) {
	// find first left paren, then find matching paren
	let lparen;
	do {
...
parser.prototype._handleExponentiation = function (tokenArr) {
	let powIdx;
	do {
...
parser.prototype._parseHelper = function (tokenArr) {
	...
	// operators with lower precedence to be resolved prior
...
parser.prototype._findMatchingParen = function (tokens, lparenIdx) {
	var parenStack = 1;
	var i = lparenIdx + 1;
